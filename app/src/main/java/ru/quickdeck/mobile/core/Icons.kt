 ...
 *
 * Важная тонкость, на которой знаки один раз уже сломались: в отдельном
 * <path> ведущий относительный m по спеке SVG означает абсолютные
 * координаты. При склейке он начинал отсчитываться от конца предыдущего
 * подпути, и деталь знака уезжала за его пределы — галочка вылезала из
 * рамки, стрелка из круга. Поэтому у каждого подпути ведущий m поднят в M.
 *
